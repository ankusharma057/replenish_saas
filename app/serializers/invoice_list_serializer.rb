...
class InvoiceListSerializer < ActiveModel::Serializer
  ...
  attributes :id, :employee_id, :client_id, :charge, :is_finalized, :date_of_service, :paid_by_client_cash, :paid_by_client_credit, :amt_paid_for_products,
             ...
             :gfe, :provider_purchased, :overhead_fee_type, :overhead_fee_value, :products_hash, :source_invoice_id, :employee, :client, :location,
             :is_paid, :stripe_account_id, :created_at, :instant_pay, :payment_status
  ...
  attribute :fellow_non_finalized_invoices do
    object.invoice_group.invoices.select { |invoice| !invoice.is_finalized && invoice.id != object.id }.map(&:id) if object.invoice_group
  end
end

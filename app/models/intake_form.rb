class IntakeForm < ApplicationRecord
  ...
  scope :by_appointment_type, ->(employee_id) {
    if employee_id.present?
      intake_forms_with_employee = IntakeForm.joins(:employees).where(employees: { id: employee_id })
      intake_forms_with_employee = IntakeForm.where(employee_id: employee_id) 
      intake_forms_without_employee = IntakeForm.where(appointment_type: "all_appointments")

      intake_forms_with_employee.or(intake_forms_without_employee)
    else
      all
    end
  }
  ...
  def valid_for_expired_date(response)
    return nil if valid_for == 'forever'
